...
def validate(input, *pipeline: list[Callable]) -> Callable[..., bool]:
    # The rest operator (*pipeline) is used to collect all these validator functions into a single tuple parameter pipeline.
...
#  used to unpack an iterable (such as a list, tuple, or set) into individual arguments.
